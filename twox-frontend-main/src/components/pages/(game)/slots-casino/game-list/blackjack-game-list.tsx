...
export default function BlackjackGameList() {
  const {
    games,
    loading,
    initialLoading,
  ...
  useEffect(() => {
    // Set search query to 'blackjack' to filter blackjack games
    setSearchQuery('blackjack')
        ...
        <>
          {games.length === 0 && (
            <div className='text-md flex justify-center gap-2 py-20 text-center font-bold uppercase md:text-lg'>
              <span>{t('game_list.no_games_found') || 'No blackjack games found'}</span>
            </div>
          )}
