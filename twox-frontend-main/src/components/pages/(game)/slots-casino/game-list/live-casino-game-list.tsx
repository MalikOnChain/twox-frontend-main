...
export default function LiveCasinoGameList() {
  const {
    games,
    loading,
    initialLoading,
  ...
  useEffect(() => {
    // Set search query to 'live' to filter live casino games
    setSearchQuery('live')
      ...
      <div className='flex flex-wrap items-center justify-end gap-2 pb-8 sm:flex-nowrap md:justify-between md:gap-3'>
        <div className='flex items-center gap-2'>
          <h1 className='text-lg font-bold uppercase md:text-xl'>
            {t('navbar.live_games') || 'Live Casino'}
          </h1>
        </div>
      </div>
        ...
        <>
          {games.length === 0 && (
            <div className='text-md flex justify-center gap-2 py-20 text-center font-bold uppercase md:text-lg'>
              <span>{t('game_list.no_games_found') || 'No live casino games found'}</span>
            </div>
          )}
