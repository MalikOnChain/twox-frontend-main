...
import { getRecentGames } from '@/api/user-games'
  ...
  const fetchRecentGames = async () => {
    try {
      ...
      if (err instanceof Error) {
        ...
        setError('Error while getting recent games')
        toast.error('Error while getting recent games')
      }
    } finally {
      setLoading(false)
    }
    ...
    return (
      <div>
        ...
        <div className='flex flex-col items-center justify-center py-20'>
          <p className='text-md mb-6'>Please sign in to view your recently played games</p>
          <button
        ...
        <>
          {games.length === 0 && (
            <div className='text-md flex justify-center gap-2 py-20 text-center font-bold uppercase md:text-lg'>
              <span>{t('game_list.no_games_found') || 'No recently played games yet. Start playing to see them here!'}</span>
            </div>
          )}
