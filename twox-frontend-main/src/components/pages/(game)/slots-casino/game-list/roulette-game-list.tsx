...
export default function RouletteGameList() {
  const {
    games,
    loading,
    initialLoading,
  ...
  useEffect(() => {
    // Set search query to 'roulette' to filter roulette games
    setSearchQuery('roulette')
        ...
        <>
          {games.length === 0 && (
            <div className='text-md flex justify-center gap-2 py-20 text-center font-bold uppercase md:text-lg'>
              <span>{t('game_list.no_games_found') || 'No roulette games found'}</span>
            </div>
          )}
